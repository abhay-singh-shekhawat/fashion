...
export default function ItemCard({ item, onClick, selected = false, selectable = false }) {
  ...
  return (
    <Tag
      ...
      onClick={onClick}
      className={`glass press group overflow-hidden p-0 text-left ${
        selected ? 'ring-2 ring-brand-primary' : 'hover-lift'
      }`}
    >
      {/* Portrait crop: clothing reads better tall, and the ratio keeps every
          tile the same shape across the 2-to-5 column range. */}
      <div className="relative grid aspect-[4/5] w-full place-items-center bg-gradient-to-br from-white/[0.06] to-transparent">
        {item.imageUrl ? (
          <img
            src={item.imageUrl}
            alt={item.name}
